...
export default function JobDetails() {
  ...
  const branchLabel = job ? (job.branch?.name || job.branch?.city || job.branch) : '';
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', fontSize: 14, color: 'var(--text-secondary)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><FiMapPin /> {branchLabel}</span>
  ...
  if (loading) {
    if (isApplicant) return <ApplicantLayout><Loader /></ApplicantLayout>;
    return <><Navbar /><Loader /></>;
  }

  if (error) {
    if (isApplicant) return <ApplicantLayout>{errorBody}</ApplicantLayout>;
    return <><Navbar />{errorBody}</>;
  }

  if (isApplicant) {
    return <ApplicantLayout>{detailBody}</ApplicantLayout>;
  }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <Navbar />
      <div style={{ margin: '40px auto', padding: '0 24px', flex: 1, width: '100%', display: 'flex', justifyContent: 'center' }}>
        {detailBody}
      </div>
      <Footer />
    </div>
