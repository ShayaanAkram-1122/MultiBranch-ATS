...
export default function Jobs() {
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((j) => {
      ...
      const b = (j.branch?.name || j.branch?.city || j.branch || "").toLowerCase();
  ...
  const inner = (
    <>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24, maxWidth: 1100 }}>
        {BRANCHES.map((b) => (
          <button
            key={b}
                ...
                <div className="card-body">
                  ...
                  >
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <FiMapPin /> {job.branch?.name || job.branch?.city || job.branch || "—"}
                    </span>
                    {job.salaryRange && (
  ...
  if (isApplicant) {
    return <ApplicantLayout>{inner}</ApplicantLayout>;
  }

  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      <Navbar />
      <main style={{ flex: 1 }}>{inner}</main>
      <Footer minimal />
    </div>
  );
}
